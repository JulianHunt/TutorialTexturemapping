...
ConeTexturedModel::ConeTexturedModel(float height, float radius, int nFaces, WCHAR** pTextureFileNames) : GameModel()
{
	InitializeModel(height, radius, nFaces, pTextureFileNames);
}

ConeTexturedModel::~ConeTexturedModel(void)
{ 
}


void ConeTexturedModel::InitializeModel(float height, float radius, int nFaces, WCHAR** pTextureFileNames)
{
	...
	*/
	...
	m_textureFileNames = new WCHAR*[NUMBER_OF_TEXTURES-1]; //file names of 3 face .dds texture files
	...
	//temporary vertices for top and bottom
 	XMFLOAT3* topVertices = new XMFLOAT3[1];
	...
	m_VertexModelArray = new ModelClass*[NUMBER_OF_TEXTURES-1]; 
    ...
    for (int i = 0; i < numberOfFaces; i++)
    {
		   ...
		   m_textureVertices[3*i+0].position = bottomVertices[i];  //bottom left
	       m_textureVertices[3*i+0].texture = XMFLOAT2(faceWidth * i, 1.0f);	
	       m_textureVertices[3*i+1].position = topCenter;   //top right
	       m_textureVertices[3*i+1].texture = XMFLOAT2(faceWidth * (i + 1), 0.0f);
	       m_textureVertices[3*i+2].position = bottomVertices[i+1];   //bottom right
	       m_textureVertices[3*i+2].texture = XMFLOAT2(faceWidth * (i + 1), 1.0f);
...
bool ConeTexturedModel::InitializeVertexModels(ID3D11Device* d3dDevice){
...
bool ConeTexturedModel::initializeTextures(ID3D11Device* d3dDevice){
	
	bool result;

	// Create quad texture object.
	const int NUMBER_OF_TEXTURES = 3; //one for the sides and one each for top and bottom
	m_faceTextures = new TextureClass*[NUMBER_OF_TEXTURES-1];
	if(!m_faceTextures)
	{
		return false;
	}
...
ID3D11ShaderResourceView* ConeTexturedModel::GetTexture(int i){
...
bool ConeTexturedModel::Render(ID3D11DeviceContext* deviceContext,  XMFLOAT4X4 viewMatrix, XMFLOAT4X4 projectionMatrix, ColorShaderClass* colorShader, TextureShaderClass* textureShader){
...
void ConeTexturedModel::Shutdown()
{
